...
class SunsynkClient:
    ...
    def get_plant_inverters(self, plant_id) -> list:
        """Return the list of inverters registered under a plant.

        This is the correct endpoint for discovering inverter serial numbers.
        Each inverter dict has 'sn' (serial number) and 'alias' (display name).
        Use this instead of parsing get_plant_realtime() which does not reliably
        include inverter SNs in all API versions.
        """
        data = self._get(f"v1/plant/{plant_id}/inverters?page=1&limit=20")
        if not data:
            return []
        # Response is either a list or {"infos": [...]}
...
def poll(site: dict, client: SunsynkClient) -> list[dict]:
    ...
    # Step 1: discover inverter SNs from the plant inverters endpoint
    # This is the authoritative list — one SN per physical inverter.
    ...
    # Step 2: fallback — try the plant realtime endpoint (older API versions)
    if not inverter_sns:
    ...
    # Step 3: fallback — manually configured SNs in the site's inverters JSONB
    # (set via Sites → Edit → Inverters in the web UI)
    if not inverter_sns:
        inverter_sns = [
            i["inverter_sn"]
    ...
    if inverter_sns:
        log.debug(f"[{site_name}] Found {len(inverter_sns)} inverter(s): {inverter_sns}")
